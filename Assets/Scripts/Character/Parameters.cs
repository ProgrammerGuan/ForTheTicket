static public class Parameters
{
    public static float MoveSpeed = 0.08f;
    public static float Vx = MoveSpeed / 0.001f;
    public static float JumpHeight = 6f;
    public static float CanJump = 0;
    public static float KickCoolDownTime = 1.3f;
    public static float SkillTime = 3.7f;
    public static float DamageCoolDownTime = 1.7f;
    public static float ActionTime = 0f;
    public static float DamageTime = 0f;
    public static float FallTicketHeight = 6f;
    public static float NormalKickDistance = 4f;
    public static float JumpKickDistance = 6f;
    public static float GotDamageDistance = 4f;
    public static int SendDistance = 15;
    public static float KickShakeTime = 1f;
    public static float KickShakeRange = 0.1f;
    public static float SkillShakeTime = 1.5f;
    public static float SkillShakeRange = 0.5f;
}
